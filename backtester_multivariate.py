#!/usr/bin/env python3
"""
Enhanced Multivariate Backtester - Tests all feature combinations
Train on P1 (2014-2018), evaluate on P2 (2019-2023)

Reads:  clean_data/PM_plus4.csv
Writes: analysis/visuals/combo_backtest_summary.csv
        analysis/visuals/combo_backtest_rankings.csv
        analysis/visuals/p2_roc_curves.png
        analysis/visuals/p2_pr_curves.png
"""

import argparse
import itertools
import math
import os
import re
from pathlib import Path
from typing import Dict, List, Optional, Sequence, Tuple

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from sklearn.calibration import CalibratedClassifierCV
from sklearn.ensemble import RandomForestClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import (
    accuracy_score,
    average_precision_score,
    confusion_matrix,
    f1_score,
    precision_recall_curve,
    precision_score,
    recall_score,
    roc_auc_score,
    roc_curve,
)
from sklearn.model_selection import StratifiedKFold, GridSearchCV
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler

# ====== CONFIG ======
RANDOM_STATE = 42
HI_POVERTY_Q = 0.70
LOW_UPTAKE_Q = 0.10
POVERTY_FLOOR = 0.15
BASE_FEATURES = [
    "pct_no_vehicle",
    "pct_no_internet",
    "pct_no_computer",
    "pct_hs_diploma_only",
]
P1_PERIOD = "P1"
P2_PERIOD = "P2"
POV_COL = "pov_fam"
SNAP_COL = "snap_fam"
LOGISTIC_C_VALUES = [0.01, 0.1, 1.0, 10.0]
PRK_FRACS = [0.01, 0.05]
# =====================


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/evaluate all feature combinations on PolicyMap data."
    )
    base_dir = Path(__file__).resolve().parent
    default_data = base_dir / "clean_data" / "PM_plus4.csv"
    default_out = base_dir / "analysis" / "visuals"
    parser.add_argument(
        "--data",
        default=str(default_data),
        help="Input CSV path (default: %(default)s).",
    )
    parser.add_argument(
        "--outdir",
        default=str(default_out),
        help="Output directory for summary + plots (default: %(default)s).",
    )
    return parser.parse_args()


def normalize_pct(series: pd.Series) -> pd.Series:
    s = pd.to_numeric(series, errors="coerce")
    if s.notna().sum() == 0:
        return s
    if (s > 1).mean(skipna=True) > 0.1:
        s = s / 100.0
    return s.clip(lower=0, upper=1)


def coalesce_columns(df: pd.DataFrame, base: str) -> pd.Series:
    pattern = re.compile(rf"^{re.escape(base)}(?:[\W_]*(?:x|y))?$", flags=re.IGNORECASE)
    candidates = [c for c in df.columns if pattern.match(c) or c.lower() == base.lower()]
    if not candidates:
        return pd.Series([np.nan] * len(df))
    result = pd.to_numeric(df[candidates[0]], errors="coerce")
    for col in candidates[1:]:
        result = result.where(result.notna(), pd.to_numeric(df[col], errors="coerce"))
    return result


def reshape_period_wide_to_long(df: pd.DataFrame) -> pd.DataFrame:
    period_pattern = re.compile(r"^(?P<base>.+)_P(?P<period>\d+)$", flags=re.IGNORECASE)
    period_map: Dict[str, Dict[str, str]] = {}
    for col in df.columns:
        match = period_pattern.match(col)
        if match:
            base = match.group("base")
            period = f"P{match.group('period')}"
            period_map.setdefault(period.upper(), {})[base] = col
    if not period_map:
        return df.copy()
    id_cols = [c for c in df.columns if not period_pattern.match(c)]
    frames = []
    for period, mapping in sorted(period_map.items()):
        frame = df[id_cols].copy()
        for base, col in mapping.items():
            frame[base] = pd.to_numeric(df[col], errors="coerce")
        frame["period"] = period
        frames.append(frame)
    long_df = pd.concat(frames, ignore_index=True)
    long_df.replace({"": np.nan, " ": np.nan}, inplace=True)
    return long_df


def ensure_long_format(df: pd.DataFrame) -> pd.DataFrame:
    if "period" in df.columns:
        long_df = df.copy()
    else:
        long_df = reshape_period_wide_to_long(df)
    long_df["period"] = long_df["period"].astype(str).str.upper()
    for base in [POV_COL, SNAP_COL] + BASE_FEATURES:
        if base not in long_df.columns:
            long_df[base] = coalesce_columns(long_df, base)
    long_df[POV_COL] = normalize_pct(long_df[POV_COL])
    long_df[SNAP_COL] = normalize_pct(long_df[SNAP_COL])
    for feat in BASE_FEATURES:
        long_df[feat] = normalize_pct(long_df[feat])
    return long_df


def compute_target(
    df: pd.DataFrame,
    low_uptake_q: float = LOW_UPTAKE_Q,
    hi_q: float = HI_POVERTY_Q,
    poverty_floor: float = POVERTY_FLOOR,
) -> Tuple[pd.Series, Dict[str, float]]:
    pov = df[POV_COL]
    snap = df[SNAP_COL]
    uptake = snap / pov.replace(0, np.nan)
    eligible = pov.notna() & snap.notna() & (pov >= poverty_floor) & uptake.notna() & (uptake > 0)
    if eligible.any():
        hi_thr = float(np.nanquantile(pov[eligible], hi_q))
        lo_thr = float(np.nanquantile(uptake[eligible], low_uptake_q))
    else:
        hi_thr = math.nan
        lo_thr = math.nan
    y = pd.Series(np.nan, index=df.index)
    y.loc[eligible] = 0.0
    positives = eligible & (pov >= hi_thr) & (uptake <= lo_thr)
    y.loc[positives] = 1.0
    meta = {
        "n_total": int(len(df)),
        "n_eligible": int(eligible.sum()),
        "n_valid": int(y.notna().sum()),
        "pos_rate": float(y.dropna().mean()) if y.notna().any() else math.nan,
        "hi_poverty_threshold": hi_thr,
        "low_uptake_threshold": lo_thr,
    }
    return y, meta


def prevalence_threshold(y_true, y_proba):
    """Set threshold to match prevalence in training data"""
    target_prevalence = y_true.mean()
    sorted_proba = np.sort(y_proba)[::-1]
    n_positive = int(len(y_true) * target_prevalence)
    if n_positive >= len(sorted_proba):
        return 0.0
    threshold = sorted_proba[n_positive]
    return threshold


def precision_at_k(y_true, y_proba, k_pct):
    """Precision at top k% of predictions"""
    k = int(len(y_true) * k_pct)
    if k == 0:
        return np.nan
    top_k_idx = np.argsort(y_proba)[::-1][:k]
    return y_true[top_k_idx].mean()


def safe_roc_auc(y_true: np.ndarray, scores: np.ndarray) -> float:
    try:
        if len(np.unique(y_true)) < 2:
            return math.nan
        return float(roc_auc_score(y_true, scores))
    except ValueError:
        return math.nan


def safe_average_precision(y_true: np.ndarray, scores: np.ndarray) -> float:
    try:
        return float(average_precision_score(y_true, scores))
    except ValueError:
        return math.nan


def train_and_evaluate_combo(
    train_df: pd.DataFrame,
    test_df: pd.DataFrame,
    y_train: pd.Series,
    y_test: pd.Series,
    feature_combo: Tuple[str, ...],
    model_type: str,
) -> Dict[str, object]:
    """
    Train and evaluate a single feature combination
    Returns dict with all metrics
    """
    # Filter valid samples
    train_mask = y_train.notna()
    test_mask = y_test.notna()
    for feat in feature_combo:
        train_mask &= train_df[feat].notna()
        test_mask &= test_df[feat].notna()
    
    if train_mask.sum() < 30 or len(np.unique(y_train[train_mask].astype(int))) < 2:
        return {
            "model": model_type,
            "feature_set": "+".join(feature_combo),
            "n_features": len(feature_combo),
            "p1_train_size": int(train_mask.sum()),
            "p2_test_size": int(test_mask.sum()),
            "p1_train_auc": math.nan,
            "p2_auc": math.nan,
        }
    
    X_train = train_df.loc[train_mask, list(feature_combo)].values
    y_train_arr = y_train.loc[train_mask].astype(int).values
    X_test = test_df.loc[test_mask, list(feature_combo)].values
    y_test_arr = y_test.loc[test_mask].astype(int).values
    
    if test_mask.sum() < 10 or len(np.unique(y_test_arr)) < 2:
        test_valid = False
    else:
        test_valid = True
    
    # Train model
    if model_type == "logistic":
        # GridSearch for best C
        best_c = None
        best_ap = -np.inf
        cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=RANDOM_STATE)
        for C in LOGISTIC_C_VALUES:
            pipeline = Pipeline([
                ("scaler", StandardScaler()),
                ("clf", LogisticRegression(
                    C=C, class_weight="balanced", max_iter=1000, random_state=RANDOM_STATE
                ))
            ])
            ap_scores = []
            for train_idx, val_idx in cv.split(X_train, y_train_arr):
                pipeline.fit(X_train[train_idx], y_train_arr[train_idx])
                val_scores = pipeline.predict_proba(X_train[val_idx])[:, 1]
                ap_scores.append(safe_average_precision(y_train_arr[val_idx], val_scores))
            mean_ap = np.nanmean(ap_scores)
            if mean_ap > best_ap:
                best_ap = mean_ap
                best_c = C
        
        # Train final model with best C
        pipeline = Pipeline([
            ("scaler", StandardScaler()),
            ("clf", LogisticRegression(
                C=best_c, class_weight="balanced", max_iter=1000, random_state=RANDOM_STATE
            ))
        ])
        pipeline.fit(X_train, y_train_arr)
        
        # Light calibration
        cal_model = CalibratedClassifierCV(pipeline, cv=3, method='isotonic')
        cal_model.fit(X_train, y_train_arr)
        model = cal_model
        
        # Get coefficients
        coefs = pipeline.named_steps["clf"].coef_.ravel()
        coef_dict = {f"coef_{feat}": val for feat, val in zip(feature_combo, coefs)}
        
    else:  # random_forest
        model = RandomForestClassifier(
            n_estimators=1000,
            min_samples_leaf=5,
            max_depth=10,
            class_weight='balanced',
            random_state=RANDOM_STATE,
            n_jobs=-1,
        )
        model.fit(X_train, y_train_arr)
        
        # Light calibration
        cal_model = CalibratedClassifierCV(model, cv=3, method='isotonic')
        cal_model.fit(X_train, y_train_arr)
        model = cal_model
        coef_dict = {}
    
    # Train metrics
    train_proba = model.predict_proba(X_train)[:, 1]
    train_auc = safe_roc_auc(y_train_arr, train_proba)
    train_ap = safe_average_precision(y_train_arr, train_proba)
    
    # Prevalence-aligned threshold
    threshold = prevalence_threshold(y_train_arr, train_proba)
    train_pred = (train_proba >= threshold).astype(int)
    train_precision = float(precision_score(y_train_arr, train_pred, zero_division=0))
    train_recall = float(recall_score(y_train_arr, train_pred, zero_division=0))
    train_f1 = float(f1_score(y_train_arr, train_pred, zero_division=0))
    
    # Train confusion matrix
    tn, fp, fn, tp = confusion_matrix(y_train_arr, train_pred).ravel()
    train_cm = {"tn": int(tn), "fp": int(fp), "fn": int(fn), "tp": int(tp)}
    
    # Train PR@k
    train_pr_at_1pct = precision_at_k(y_train_arr, train_proba, 0.01)
    train_pr_at_5pct = precision_at_k(y_train_arr, train_proba, 0.05)
    
    result = {
        "model": model_type,
        "feature_set": "+".join(feature_combo),
        "n_features": len(feature_combo),
        "p1_train_size": int(train_mask.sum()),
        "p1_train_auc": train_auc,
        "p1_train_ap": train_ap,
        "p1_train_precision": train_precision,
        "p1_train_recall": train_recall,
        "p1_train_f1": train_f1,
        "p1_train_pr_at_1pct": train_pr_at_1pct,
        "p1_train_pr_at_5pct": train_pr_at_5pct,
        "p1_threshold": threshold,
        **coef_dict,
    }
    
    if not test_valid:
        result.update({
            "p2_test_size": int(test_mask.sum()),
            "p2_auc": math.nan,
            "p2_ap": math.nan,
            "p2_precision": math.nan,
            "p2_recall": math.nan,
            "p2_f1": math.nan,
            "p2_accuracy": math.nan,
            "p2_pr_at_1pct": math.nan,
            "p2_pr_at_5pct": math.nan,
        })
        return result
    
    # Test metrics
    test_proba = model.predict_proba(X_test)[:, 1]
    test_auc = safe_roc_auc(y_test_arr, test_proba)
    test_ap = safe_average_precision(y_test_arr, test_proba)
    
    test_pred = (test_proba >= threshold).astype(int)
    test_precision = float(precision_score(y_test_arr, test_pred, zero_division=0))
    test_recall = float(recall_score(y_test_arr, test_pred, zero_division=0))
    test_f1 = float(f1_score(y_test_arr, test_pred, zero_division=0))
    test_accuracy = float(accuracy_score(y_test_arr, test_pred))
    
    # Test confusion matrix
    tn, fp, fn, tp = confusion_matrix(y_test_arr, test_pred).ravel()
    test_cm = {"tn": int(tn), "fp": int(fp), "fn": int(fn), "tp": int(tp)}
    
    # Test PR@k
    test_pr_at_1pct = precision_at_k(y_test_arr, test_proba, 0.01)
    test_pr_at_5pct = precision_at_k(y_test_arr, test_proba, 0.05)
    
    result.update({
        "p2_test_size": int(test_mask.sum()),
        "p2_auc": test_auc,
        "p2_ap": test_ap,
        "p2_precision": test_precision,
        "p2_recall": test_recall,
        "p2_f1": test_f1,
        "p2_accuracy": test_accuracy,
        "p2_pr_at_1pct": test_pr_at_1pct,
        "p2_pr_at_5pct": test_pr_at_5pct,
    })
    
    # Store for plotting if it's the full feature set
    if len(feature_combo) == len(BASE_FEATURES) and set(feature_combo) == set(BASE_FEATURES):
        result["_plot_data"] = (y_test_arr, test_proba)
    
    return result


def test_all_combinations(
    train_df: pd.DataFrame,
    test_df: pd.DataFrame,
    y_train: pd.Series,
    y_test: pd.Series,
) -> pd.DataFrame:
    """Test all feature combinations with both models"""
    results = []
    
    # All logistic regression combinations (1 to n features)
    for r in range(1, len(BASE_FEATURES) + 1):
        for combo in itertools.combinations(BASE_FEATURES, r):
            print(f"  Testing LR: {' + '.join(combo)}")
            result = train_and_evaluate_combo(
                train_df, test_df, y_train, y_test, combo, "logistic"
            )
            results.append(result)
    
    # Random forest with all features
    print(f"  Testing RF: all features")
    result = train_and_evaluate_combo(
        train_df, test_df, y_train, y_test, tuple(BASE_FEATURES), "random_forest"
    )
    results.append(result)

    return pd.DataFrame(results)


def plot_curves(
    results_df: pd.DataFrame,
    out_dir: Path,
) -> None:
    """Plot ROC and PR curves for full feature set models"""
    lr_data = None
    rf_data = None
    
    for _, row in results_df.iterrows():
        if "_plot_data" in row and row["_plot_data"] is not None:
            if row["model"] == "logistic":
                lr_data = row["_plot_data"]
            elif row["model"] == "random_forest":
                rf_data = row["_plot_data"]
    
    if lr_data is None and rf_data is None:
        return
    
    # ROC curve
    plt.figure(figsize=(8, 6))
    if lr_data is not None and len(np.unique(lr_data[0])) > 1:
        fpr, tpr, _ = roc_curve(lr_data[0], lr_data[1])
        auc = roc_auc_score(lr_data[0], lr_data[1])
        plt.plot(fpr, tpr, label=f"Logistic (AUC={auc:.3f})")
    if rf_data is not None and len(np.unique(rf_data[0])) > 1:
        fpr, tpr, _ = roc_curve(rf_data[0], rf_data[1])
        auc = roc_auc_score(rf_data[0], rf_data[1])
        plt.plot(fpr, tpr, label=f"Random Forest (AUC={auc:.3f})")
    plt.plot([0, 1], [0, 1], "k--", alpha=0.3)
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("P2 ROC Curves – All Features")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_dir / "p2_roc_curves.png", dpi=200)
    plt.close()
    
    # PR curve
    plt.figure(figsize=(8, 6))
    if lr_data is not None:
        precision, recall, _ = precision_recall_curve(lr_data[0], lr_data[1])
        ap = average_precision_score(lr_data[0], lr_data[1])
        plt.plot(recall, precision, label=f"Logistic (AP={ap:.3f})")
    if rf_data is not None:
        precision, recall, _ = precision_recall_curve(rf_data[0], rf_data[1])
        ap = average_precision_score(rf_data[0], rf_data[1])
        plt.plot(recall, precision, label=f"Random Forest (AP={ap:.3f})")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("P2 Precision-Recall Curves – All Features")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_dir / "p2_pr_curves.png", dpi=200)
    plt.close()


def main() -> None:
    args = parse_args()
    data_path = Path(args.data).expanduser()
    out_dir = Path(args.outdir).expanduser()
    out_dir.mkdir(parents=True, exist_ok=True)

    # Load and prepare data
    print("Loading data...")
    df = pd.read_csv(data_path, low_memory=False)
    df.replace({"": np.nan, " ": np.nan}, inplace=True)
    df_long = ensure_long_format(df)

    missing_rates = {
        feat: float(df_long[feat].isna().mean())
        for feat in BASE_FEATURES
    }
    print("Missing data rates (share of NaNs) per feature:", missing_rates)

    p1_df = df_long[df_long["period"] == P1_PERIOD].reset_index(drop=True)
    p2_df = df_long[df_long["period"] == P2_PERIOD].reset_index(drop=True)

    if p1_df.empty or p2_df.empty:
        raise ValueError("Could not locate both P1 and P2 rows in the dataset.")

    mask_p1 = p1_df[BASE_FEATURES].notna().all(axis=1)
    mask_p2 = p2_df[BASE_FEATURES].notna().all(axis=1)
    print(f"P1 rows before mask: {len(p1_df)}  after mask: {mask_p1.sum()}")
    print(f"P2 rows before mask: {len(p2_df)}  after mask: {mask_p2.sum()}")
    p1_df = p1_df.loc[mask_p1].reset_index(drop=True)
    p2_df = p2_df.loc[mask_p2].reset_index(drop=True)

    # Compute targets
    print("Computing targets...")
    y_p1, meta_p1 = compute_target(p1_df)
    y_p2, meta_p2 = compute_target(p2_df)
    
    print(f"P1: {meta_p1['n_valid']} valid samples, {meta_p1['pos_rate']:.3f} positive rate")
    print(f"P2: {meta_p2['n_valid']} valid samples, {meta_p2['pos_rate']:.3f} positive rate")
    
    # Test all combinations
    print("\nTesting all feature combinations...")
    results_df = test_all_combinations(p1_df, p2_df, y_p1, y_p2)

    # Add metadata
    results_df["hi_poverty_quantile"] = HI_POVERTY_Q
    results_df["low_uptake_quantile"] = LOW_UPTAKE_Q
    results_df["poverty_floor"] = POVERTY_FLOOR
    results_df["p1_pos_rate"] = meta_p1["pos_rate"]
    results_df["p2_pos_rate"] = meta_p2["pos_rate"]
    
    # Sort by P2 AUC
    results_df.sort_values(by="p2_auc", ascending=False, inplace=True)
    
    # Remove plot data column before saving
    auc_by_features = (
        results_df.dropna(subset=["p2_auc"])
        .groupby("n_features")["p2_auc"]
        .max()
        .reset_index()
        .sort_values("n_features")
    )
    if not auc_by_features.empty:
        plt.figure(figsize=(6, 4))
        plt.bar(auc_by_features["n_features"], auc_by_features["p2_auc"])
        plt.xlabel("Number of features")
        plt.ylabel("Best P2 AUC")
        plt.title("P2 AUC by Feature Count")
        plt.tight_layout()
        auc_bar_path = out_dir / "combo_auc_by_feature_count.png"
        plt.savefig(auc_bar_path, dpi=200)
        plt.close()
        print(f"[write] {auc_bar_path}")

    best_logistic = (
        results_df[results_df["model"] == "logistic"]
        .dropna(subset=["p2_auc"])
        .sort_values("p2_auc", ascending=False)
        .head(1)
    )
    if not best_logistic.empty:
        row = best_logistic.iloc[0]
        coef_cols = [c for c in best_logistic.columns if c.startswith("coef_")]
        coefs = row[coef_cols].dropna()
        if not coefs.empty:
            features = [c.replace("coef_", "") for c in coefs.index]
            values = coefs.values.astype(float)
            plt.figure(figsize=(6, 4))
            plt.barh(features, values)
            plt.xlabel("Coefficient (standardised units)")
            plt.title(f"Logistic coefficients – best combo ({row['feature_set']})")
            plt.tight_layout()
            coef_path = out_dir / "combo_best_logistic_coefficients.png"
            plt.savefig(coef_path, dpi=200)
            plt.close()
            print(f"[write] {coef_path}")

    if "_plot_data" in results_df.columns:
        plot_data = results_df["_plot_data"].copy()
        results_df = results_df.drop(columns=["_plot_data"])
    
    # Save summary
    summary_path = out_dir / "combo_backtest_summary.csv"
    results_df.to_csv(summary_path, index=False)
    print(f"\n[write] {summary_path}")
    
    # Create rankings
    rankings = []
    for scope_name in ["Top_by_AUC", "Top_by_AP", "Top_by_F1"]:
        if scope_name == "Top_by_AUC":
            sorted_df = results_df.sort_values(by="p2_auc", ascending=False)
        elif scope_name == "Top_by_AP":
            sorted_df = results_df.sort_values(by="p2_ap", ascending=False)
        else:
            sorted_df = results_df.sort_values(by="p2_f1", ascending=False)
        
        for rank, (_, row) in enumerate(sorted_df.head(10).iterrows(), 1):
            ranking_row = row.to_dict()
            ranking_row["ranking_scope"] = scope_name
            ranking_row["rank"] = rank
            rankings.append(ranking_row)
    
    rankings_df = pd.DataFrame(rankings)
    rankings_path = out_dir / "combo_backtest_rankings.csv"
    rankings_df.to_csv(rankings_path, index=False)
    print(f"[write] {rankings_path}")
    
    # Plot curves
    print("Generating plots...")
    # Restore plot data
    if "_plot_data" in locals():
        results_df["_plot_data"] = plot_data
    plot_curves(results_df, out_dir)
    print(f"[write] {out_dir / 'p2_roc_curves.png'}")
    print(f"[write] {out_dir / 'p2_pr_curves.png'}")
    
    # Print summary
    print("\n=== SUMMARY ===")
    print(f"\nTop 5 by P2 AUC:")
    display_cols = ["model", "feature_set", "n_features", "p2_auc", "p2_ap", 
                    "p2_precision", "p2_recall", "p2_f1"]
    print(results_df[display_cols].head(5).to_string(index=False))
    
    print("\nMultivariate combination backtesting complete.")


if __name__ == "__main__":
    main()
